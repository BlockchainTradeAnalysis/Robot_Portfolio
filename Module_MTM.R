...
for(j in 1:3){
  ...
  for(k in 1:14){
    ...
    check_buy=1
    for(i in 2:nrow(STK)){
      if(check_buy==1 && m_MTM_STK[i]>0.005 && a_MTM_STK[i]/a_MTM_STK[i-1] > 1.73){ #60度
        buy_loc = c(buy_loc,i)
        check_buy = 0
      }
      if(check_buy==0 && m_MTM_STK[i]<(-0.005) && a_MTM_STK[i]/a_MTM_STK[i-1] > 1.19){ #50度
...
#合併結果
result_module_MTM <- data.frame(Time_ID=c(1:14),
...
save(result_module_MTM, file="C:\\RData\\result_module_MTM_manybuy_onesell_BTCUSDT_5MIN_2017_8_17_2018_12_15.rda")
